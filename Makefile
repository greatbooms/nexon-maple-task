.PHONY: setup start stop status init restart clean help seed-data all

help:
	@echo "MongoDB 레플리카 세트 관리 명령어:"
	@echo "  make setup    - 키파일 생성 및 스크립트에 실행 권한 부여"
	@echo "  make start    - MongoDB 컨테이너 시작"
	@echo "  make init     - 레플리카 세트 초기화"
	@echo "  make status   - 레플리카 세트 상태 확인"
	@echo "  make stop     - MongoDB 컨테이너 중지"
	@echo "  make restart  - MongoDB 컨테이너 재시작"
	@echo "  make clean    - 모든 컨테이너와 볼륨 제거"
	@echo "  make seed-data - Point, Item, Coupon 컬렉션에 샘플 데이터 생성"
	@echo "  make all      - 전체 설정 및 샘플 데이터 생성"
...
# 순차적 실행을 위한 전체 설정 명령어 (seed-data 포함)
all: setup start init seed-data
	@echo "MongoDB 레플리카 세트가 완전히 설정되었고, 샘플 데이터가 생성되었습니다!"
